...
This module creates comprehensive visualizations for healthcare network analysis,
including static plots (matplotlib) and interactive dashboards (Plotly).

Types:
1. PageRank Analysis: Distribution, rankings, top providers
2. Risk Analysis: Risk propagation, VaR, Expected Shortfall
3. Network Topology: Graph visualization with NetworkX
4. Convergence Analysis: Algorithm performance metrics
5. Interactive Dashboard: 6-panel Plotly dashboard with hover tooltips
...
# Professional color scheme for healthcare visualizations
HEALTHCARE_COLORS = {
    'primary': '#2c3e50',      # Dark blue-gray (text, primary elements)
    'secondary': '#3498db',    # Blue (PageRank, positive metrics)
    'accent': '#e74c3c',       # Red (risk, warnings, high values)
    'success': '#27ae60',      # Green (success, low risk)
    'warning': '#f39c12',      # Orange (caution, medium values)
    'info': '#9b59b6',         # Purple (information, performance)
    'light': '#ecf0f1',        # Light gray (backgrounds, low values)
    'dark': '#34495e'          # Dark gray (edges, secondary elements)
}
...
def get_viz_path(filename):
    """
    Constructs full path for saving visualization files.

    Centralizes path management for all visualizations, making it easy to
    change the output directory if needed.

    Args:
        filename (str): Name of the visualization file (e.g., 'pagerank_analysis.png')

    Returns:
        Path: Full path to save the visualization
    """
    return VISUALIZATIONS_DIR / filename

def create_network_topology_plot(G, pagerank_values, index_to_npi=None, max_nodes=100):
    """
    Creates a network topology visualization showing provider connections and PageRank 
    importance.
...
def create_healthcare_analysis_plots(G, pagerank_result, provider_results):
    """
    Creates comprehensive 6-panel PageRank analysis visualization.

    This creates a single figure with 6 complementary views of the PageRank results:

    1. PageRank by Rank: Shows power-law distribution (log scale)
    2. Score Distribution: Histogram of PageRank values
    3. Network Overview: Key network statistics
    4. Top 10 Providers: Highest-ranked providers
    5. Provider Categories: Pie chart of hubs/authorities/central
    6. Algorithm Performance: Convergence metrics

    Args:
        G (scipy.sparse.csc_matrix): Network adjacency matrix (for metadata)
        pagerank_result (PageRankResult): PageRank computation results
        provider_results (dict): Network structure analysis results

    Saves:
        healthcare_pagerank_analysis.png: 6-panel analysis visualization (300 DPI)
    """
    try:
        ...
        # Panel 1: PageRank Values by Rank (log scale)
        ax1 = fig.add_subplot(gs[0, 0])
        ax1.set_title('PageRank Values by Provider Rank', fontsize=12)

        # Sample data for large networks to improve performance and clarity
        if len(values) > 200:
            top_indices = np.argsort(values)[-100:]  # Top 100 providers
            remaining_indices = np.argsort(values)[:-100]
            step = max(1, len(remaining_indices) // 100)
            spaced_indices = remaining_indices[::step][:100]  # Evenly spaced sample
        ...
        ax1.set_yscale('log')  # Log scale for power-law distribution
...
def create_risk_analysis_plots(risk_results, title="Healthcare Risk Analysis"):
    """
    Creates comprehensive 4-panel risk analysis visualization.

    This creates a multi-panel view of risk propagation results:

    1. Risk Distribution: Histogram showing spread of risk scores
    2. Risk by Rank: Log-scale plot showing concentration
    3. Key Statistics: VaR, Expected Shortfall, mean/max risk
    4. Risk Categories: Pie chart of high-risk vs low-risk providers

    - VaR (Value at Risk): 95th percentile risk score
      "95% of providers have risk below this level"
    - Expected Shortfall: Average risk among worst 5%
      "Average risk in the tail of the distribution"
    ...
    Saves:
        healthcare_risk_analysis.png: 4-panel risk analysis visualization (300 DPI)
    """
    try:
...
def create_convergence_analysis_plots(convergence_results):
    """
    Creates 2-panel algorithm convergence analysis visualization.

    This demonstrates the accuracy-speed tradeoff in iterative algorithms:

    Panel 1: Iterations vs Tolerance
    - Shows how tighter tolerance requires more iterations
    - Reveals diminishing returns (exponential relationship)

    Panel 2: Computation Time vs Tolerance
    - Shows actual wall-clock time impact
    - Helps choose optimal tolerance for production use
    ...
    Saves:
        convergence_analysis.png: 2-panel convergence performance visualization (300 DPI)
    """
    try:
...
def create_comprehensive_dashboard():
    """
    Creates interactive 6-panel Plotly dashboard for healthcare network analysis.

    This creates an HTML dashboard with interactive features:
    - Hover tooltips: Detailed information on mouse-over
    - Zoom/pan: Explore specific regions of plots
    - Responsive layout: Adapts to different screen sizes
    - Professional styling: Publication-ready appearance

    Panels:
    1. PageRank Values by Rank: Log-scale line plot showing distribution
    2. PageRank Distribution: Histogram with probability density
    3. Risk Score Distribution: Histogram of propagated risk
    4. Risk Propagation: Scatter plot showing original vs propagated risk
    5. Convergence Analysis: Performance across tolerance levels
    6. Risk Categories: Donut chart of high-risk vs low-risk providers

    Saves:
        healthcare_network_dashboard.html: Interactive dashboard (single HTML file)

    Note:
        Requires Plotly. Uses global _dashboard_data populated by add_to_dashboard().
    """
    global _dashboard_data
    try:
        ...
        # Create 6-panel subplot layout with mixed chart types
        fig = make_subplots(
            rows=3, cols=2,
            subplot_titles=(
            ...
            specs=[
                ...
                [{"secondary_y": False}, {"secondary_y": False}],
                [{"secondary_y": False}, {"type": "pie"}]  # Pie chart requires special spec
            ],
            vertical_spacing=0.18,  # Spacing between rows
            horizontal_spacing=0.12  # Spacing between columns
        )

        # Panel 1: PageRank Values by Rank
        if _dashboard_data['pagerank_data']:
            for _, pr_values_dict in _dashboard_data['pagerank_data'].items():
                ...
                # Sample data for large networks to improve rendering performance
                if len(values) > 200:
                    top_indices = np.argsort(values)[-100:]  # Always show top providers
                    remaining_indices = np.argsort(values)[:-100]
                    step = max(1, len(remaining_indices) // 100)
                    spaced_indices = remaining_indices[::step][:100]  # Evenly sample rest
                    sample_indices = np.concatenate([top_indices, spaced_indices])
                    sample_values = values[sample_indices]
                else:
                    sample_values = values
